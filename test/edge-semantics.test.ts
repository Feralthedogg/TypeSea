...
describe("edge-case runtime semantics", () => {
    ...
    test("validates direct compiled guard collector diagnostics", () => {
        ...
        const emptyStringIssues = new looseCompiledBaseGuard(
            t.string.schema,
            () => false,
            () => "",
            "source"
        ...
        const forgedTrusted = new looseCompiledBaseGuard(
            t.string.schema,
            () => false,
            () => "not_issues",
            "source",
        ...
        expect(() => emptyStringIssues.check(1)).toThrow(TypeError);
        expect(() => badIssue.check(1)).toThrow(TypeError);
        expect(() => badPathSegment.check(1)).toThrow(TypeError);
        expect(() => forgedTrusted.check(1)).toThrow(TypeError);
    ...
    test("preserves live array length semantics during item refinement", () => {
        const values: unknown[] = ["ok", 1];
        const Items = t.array(t.string.refine(
            (): boolean => {
                values.length = 1;
                return true;
            },
        ...
        const FastItems = compile(Items, { name: "liveArrayLength" });

        expect(Items.is(values)).toBe(true);
        values.length = 2;
        values[0] = "ok";
        values[1] = 1;
        expect(FastItems.is(values)).toBe(true);
        values.length = 2;
        values[0] = "ok";
        values[1] = 1;
        const interpreted = Items.check(values);
        values.length = 2;
        values[0] = "ok";
        values[1] = 1;
        const compiled = FastItems.check(values);
        expect(compiled).toEqual(interpreted);
    });
    ...
    test("rejects schema and guard records backed by prototype state", () => {
        let reads = 0;
        ...
        expect(() => new BaseGuard<string>(inheritedSchema)).toThrow(TypeError);
        expect(reads).toBe(0);

        const inheritedGuard = Object.create({
            schema: t.string.schema,
            check: (): boolean => true
        }) as unknown as Guard<string>;

        expect(() => compile(inheritedGuard, { name: "inheritedGuardSchema" }))
            .toThrow(TypeError);
        expect(() => toJsonSchema(inheritedGuard)).toThrow(TypeError);

        const inheritedCheckGuard = Object.create({
            check: (): unknown => ({
                ok: true,
                value: "ok"
            })
        }) as Record<string, unknown>;
        Object.defineProperty(inheritedCheckGuard, "schema", {
            configurable: true,
            enumerable: true,
            value: t.string.schema
        });

        expect(() => toTrpcParser(inheritedCheckGuard as unknown as Guard<string>))
            .toThrow(TypeError);
    });
...
/**
 * @brief Execute define record value.
