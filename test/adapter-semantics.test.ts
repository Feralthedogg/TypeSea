...
describe("ecosystem adapters", () => {
    ...
    test("keeps Fastify validator compiler source maps on own data properties", () => {
        const route = {
            schema: {},
            method: "POST",
            url: "/users",
        ...
        const inherited = Object.create({
            body: t.unknown
        }) as { readonly body?: typeof t.unknown };
        let reads = 0;
        const accessorSource: { readonly body?: typeof t.unknown } = {};

        Object.defineProperty(accessorSource, "body", {
            configurable: true,
            enumerable: true,
            get(): typeof t.unknown {
                reads += 1;
                return t.unknown;
            }
        });

        const inheritedCompiler = toFastifyValidatorCompiler(inherited);

        expect(() => inheritedCompiler(route)).toThrow(TypeError);
        expect(() => toFastifyValidatorCompiler(accessorSource)).toThrow(TypeError);
        expect(reads).toBe(0);
    });
...
/**
 * @brief Check react hook form errors.
 * @details Test helpers pin observable behavior so engine rewrites keep the same external result.
 */
...
/**
 * @brief Check react hook form field error.
 * @details Test helpers pin observable behavior so engine rewrites keep the same external result.
 */
