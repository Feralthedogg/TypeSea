...
[실사용 Zod 호환성 코퍼스](./zod-real-world-compat.md)는 공개 저장소 9곳에서
Zod를 import하는 파일 1,875개와 호출 28,758개를 고정된 commit 기준으로
측정합니다. import를 TypeSea로 바꿔 컴파일하는 독립 파일 224개에서 TypeSea만
추가로 발생시키는 진단은 0건이며, 관측된 declaration export 누락도 0건입니다.
이는 고정된 표본의 소스 호환성 측정값이지 Zod 전체 의미 호환을 선언하는 수치는
아닙니다.
...
마지막 clean 로컬 벤치마크는 2026-07-11 KST에 실행했습니다.
...
다룹니다.
[고정 코퍼스 문서](./zod-real-world-compat.md)에는 저장소, commit, API 사용량과
교체 컴파일 결과가 기록됩니다. 회귀 허용치는 0으로 고정되어 있지만, 이 게이트가
Zod의 비공개 parser 내부를 TypeSea 계약으로 바꾸지는 않습니다.
...
decoder가 포함된 union, intersection, lazy schema, array, object는 `Input<>`과
`Output<>` 정보를 유지합니다. object decoder는 schema 조립 단계에서 `shape`,
`extend`, `safeExtend`, `merge`, `pick`, `omit`, `partial`, `strict`, `strip`,
`passthrough`, `loose`를 계속 사용할 수 있습니다. 이 메서드들은 완성된 validation
runner에 추가 dispatch를 넣지 않습니다. guard-only object와 object decoder를
합치면 결과가 자동으로 object decoder로 승격됩니다.
`TypeSource<Output, Input, Presence>`는 guard, decoder, codec과 Zod 호환
`ZodType` alias가 공유하는 구조적 계약입니다.

`default()`는 fallback을 설치한 뒤 output type에서 `undefined`를 제거합니다.
string transform은 input type을 `string`으로 유지하고, decoder array는 child의
input array type을 보존합니다. native `t.*.refine()`은 계속 literal `true`만
성공으로 처리하며, `z.object()` facade에서만 Zod처럼 truthy refinement 결과를
허용합니다.
...
`1.2.0`은 TypeSea의 안전한 기본 검증 의미를 바꾸지 않고 소스 호환 범위를
넓힙니다. decoder child를 union, intersection, lazy, array, tuple, record, map,
set, object builder 안에서 조합해도 input/output type이 유지됩니다. metadata와
refinement를 붙인 뒤에도 object decoder의 shape 연산을 이어갈 수 있고,
guard/decoder object merge는 올바른 decoder 또는 codec 결과 타입을 선택합니다.

`typesea/zod`, `/v3`, `/v4`, `/v4/core`, `/v4-mini` declaration facade는 고정된
9개 저장소 코퍼스에서 관측된 export를 모두 제공합니다. 교체 대상 독립 파일
224개는 TypeSea만의 추가 진단 없이 컴파일됩니다. 이 수치는 측정된 migration
baseline이며 Zod parser 전체 호환을 뜻하지 않습니다.
