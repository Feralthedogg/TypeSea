...
| `z.object()`, `.strict()`, `.loose()`, `.passthrough()`, `.strip()`, `.extend()`, `.pick()`, `.omit()`, `.partial()`, `.required()` | 지원, compiled | `z.object()`는 Zod v4처럼 기본 parse output에서 unknown key를 strip합니다. safe strict object는 prototype을 읽지 않고 undeclared own string, symbol, non-enumerable key를 거부합니다. |
...
| `z.union()`, `z.discriminatedUnion()`, `z.intersection()` | 지원, compiled | branch에 required key나 discriminator가 있으면 object-union preflight가 최적화됩니다. 넓고 겹치는 union은 여전히 branch probing이 필요할 수 있습니다. |
