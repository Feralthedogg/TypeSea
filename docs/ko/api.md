...
| `z.object()`, `.strict()`, `.loose()`, `.passthrough()`, `.strip()`, `.extend()`, `.pick()`, `.omit()`, `.partial()`, `.required()` | 지원, compiled | safe strict object는 prototype을 읽지 않고 undeclared own string, symbol, non-enumerable key를 거부합니다. |
