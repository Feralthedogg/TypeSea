...
TypeSea는 신뢰할 수 없는 입력을 `unknown`으로 받고, 불변 가드 값을 통해 타입을 좁힙니다.
공개 API는 의도적으로 작게 유지합니다.
복잡한 동작은 builder validation, graph introspection, diagnostics, export check 내부에 숨겨져 있습니다.
...
| 메서드 | 용도 | 계약 |
| --- | --- | --- |
| `is` | hot boolean narrowing | 성공 경로에서 진단 할당을 피합니다. |
| `check` | issue가 필요한 validation | 얼려진 `Result<T, Issue[]>` container를 반환합니다. |
| `assert` | throw가 필요한 연동 지점 | 복사되고 얼려진 issue를 가진 `TypeSeaAssertionError`를 던집니다. |
| `graph` | runtime plan introspection | validation plan이 보유한 validated, optimized, frozen Sea-of-Nodes graph를 반환합니다. |

diagnostic path에는 object key와 0부터 시작하는 array 또는 tuple index만 들어갑니다.
공개 diagnostic validator는 잘못된 path segment를 거부한 뒤 diagnostic을 API boundary 밖으로 내보냅니다.

## Builder 계열

| 계열 | Builder |
| --- | --- |
| Scalar | `t.unknown`, `t.never`, `t.string`, `t.number`, `t.bigint`, `t.symbol`, `t.boolean` |
| Literal과 container | `t.literal(value)`, `t.array(item)`, `t.tuple([a, b])`, `t.record(value)` |
| Objects | `t.object(shape)`, `t.strictObject(shape)` |
| Object transform | `t.extend`, `t.pick`, `t.omit`, `t.partial`, and matching object guard methods |
...
| Decoder | `t.decoder`, `t.transform`, `t.pipe`, `t.coerce` |
...
TypeSea는 key presence와 value domain을 분리합니다.
...
- `nullable`, `undefinedable`, `brand`, `refine`을 지나도 presence-preserving wrapper는 optional-key semantics를 유지합니다.
...
`formatIssue`, `formatIssues`, `withMessages`는 validation이 끝난 뒤 diagnostic을 렌더링합니다.
...
`withMessages(result, options)`는 successful result를 그대로 보존하고, failed `Result`에는 복사되고 얼려진 issue에 `message` field를 채워 새로 반환합니다.
