    ...
    <div data-slot="toc" class={cn('flex flex-col', className)}>
        ...
        <nav bind:this={navEl} class="no-scrollbar flex max-h-[60vh] flex-col overflow-y-auto">
            {#each flat as item, i (item.url)}
                <a
                    href={item.url}
                    style="font-size:{fontSize(i)}px;opacity:{opacity(i)};font-weight:{activeIdx ===
                    i
                        ? 500
                        : 400};padding-block:{activeIdx === i
                        ? 5
                        : 3}px;padding-inline-start:{item.depth *
                ...
                >
                    {#if numbered}<span class="me-1.5 opacity-40">{numbering[i]}</span
                        >{/if}{item.title}
                </a>
            {/each}
        </nav>
